...
def game_over(hacker_hp, sysadmin_hp):
    if hacker_hp <= 0 and sysadmin_hp <= 0:
        ...
        return f"{blue}SysAdmin {sysadmin_name}{reset} wins the competition\n"
    elif sysadmin_hp <= 0:
        return f"{green}Hacker {hacker_name}{reset} wins the competition\n"
    else:
        return None
...
def attack_handling(hacker_sequence, sysadmin_sequence):
    ...
    while i < 3 and sequence_active:
        ...
        if hacker_val == 1:
            if hacker_en <= 15:
                print(f"{cyan_bg}Frozen, Not enough energy{reset}; {yellow}Hacker {hacker_name} sequence {i + 1} skipped{reset}")
                #hacker_sequence_skipped = True
            else:
                hacker_en -= 15
                sysadmin_hp -= 20
                print(f"{green}Hacker {hacker_name}{reset} uses DDoS attack, -20 hp to {blue}SysAdmin {sysadmin_name}{reset}")

        elif hacker_val == 2:
            if hacker_hp <= 10:
                print(f"Not enough HP; {yellow}Hacker {hacker_name} sequence {i + 1} skipped{reset}")
                #hacker_sequence_skipped = True
            else:
                hacker_hp -= 10
                hacker_en += 20
                hacker_en = min(hacker_en, 100)
                print(f"{green}{hacker_name}{reset} uses Phishing scam; +20 energy")
            ...
            if hacker_en <= 10:
                ...
                hacker_en -= 10
                hacker_stealth = True
                print(f"{green}{hacker_name}{reset} uses Stealth Mode for sequence {i + 1}")

        if sysadmin_val == 1:
            if sysadmin_en <= 15:
                print(f"{cyan_bg}Frozen, Not enough energy{reset}; {yellow}SysAdmin {sysadmin_name} sequence {i + 1} skipped{reset} \n ")
                #sysadmin_sequence_skipped = True
            else:
                sysadmin_en -= 15

                if hacker_stealth == True:
                    print(f"{blue}{sysadmin_name}{reset} uses Firewall purge, blocked by {green}Hacker {hacker_name}{reset} Stealth Mode \n")
                else:
                    hacker_hp -= 20
                    max(0, hacker_hp)
                    print(f"{blue}{sysadmin_name}{reset} uses Firewall purge, -20 hp to {green}Hacker {hacker_name}{reset} \n ")

        elif sysadmin_val == 2:
            if sysadmin_hp <= 10:
                print(f"Not enough HP; {yellow}SysAdmin {sysadmin_name} sequence {i + 1} skipped{reset} \n")
                #sysadmin_sequence_skipped = True
            else:
                sysadmin_hp -= 10
                sysadmin_en += 20
                sysadmin_en = min(sysadmin_en, 100)
                print(f"{blue}{sysadmin_name}{reset} uses Reboot system, +20 energy, -10 hp to self \n")
            ...
            if sysadmin_en <= 10:
                ...
                print(f"{blue}SysAdmin {sysadmin_name}{reset} uses Trace route,", end=" ")
                if hacker_stealth == True:
                    hacker_stealth = False
...
while game_running == 1:
    ...
    hacker_name = input(f"{green}Enter the Hacker's name: {reset}")
    hacker_name, hacker_hp, hacker_en = hacker_init(hacker_name)
    
    print("")

    sysadmin_name = input(f"{blue}Enter the SysAdmin's name: {reset}")
    ...
    while hacker_hp > 0 and sysadmin_hp > 0:
        ...
        hacker_sequence = input(f"{green}{hacker_name}{reset}, enter your attack sequence: ")

        while is_valid_sequence(hacker_sequence) == False:
            print("Invalid sequence. Try again.")
            hacker_sequence = input(f"{green}{hacker_name}{reset}, enter your attack sequence: ")

        sysadmin_sequence = input(f"{blue}{sysadmin_name}{reset}, enter your attack sequence: ")
        ...
        if turn_number % 3 == 0 and game_over(hacker_hp, sysadmin_hp) is None:
            print(f"\n{orange_red_bg}{flash}Server overheats, -10 hp to both sides{reset}\n")
            hacker_hp -= 10
            sysadmin_hp -= 10
...
while game_running == 2: 
    ...
    while hacker_hp > 0 and sysadmin_hp > 0:
        ...
        hacker_sequence = input(f"{green}Hacker {hacker_name}{reset}, enter your attack sequence: ")

        while is_valid_sequence(hacker_sequence) == False:
            print("Invalid sequence. Try again.")
            hacker_sequence = input(f"{green}Hacker {hacker_name}{reset}, enter your attack sequence: ")

        sysadmin_sequence = input(f"{blue}Sysadmin {sysadmin_name}{reset}, enter your attack sequence: ")
